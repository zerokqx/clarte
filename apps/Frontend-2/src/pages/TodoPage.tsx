...
export const TodoPage = () => {
  ...
  const {
    ...
    requestPermission,
    stats: reminderStats,
  } = useSmartReminders(tasks);

  const { projects, selectedProject, setSelectedProject, addProject, deleteProject } =
    useProjects();
  ...
  const [selectedView, setSelectedView] = useState<'Входящие' | 'Сегодня' | 'Предстоящие'>(
    'Входящие',
  );
  ...
  const [connectModalOpened, { open: openConnectModal, close: closeConnectModal }] =
    useDisclosure(false);
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const storedReadIds: string[] = JSON.parse(
        localStorage.getItem('clarte_read_notifications') || '[]',
      );
      const unread = list.filter((n: AppNotification) => !storedReadIds.includes(n.id));
      ...
      if (cached) {
        ...
        const unread = initialMockNotifs.filter(
          (n: AppNotification) => !storedReadIds.includes(n.id),
        );
        setUnreadCount(unread.length);
      }
    }
                  ...
                  sortedNotes.map((n) => (
                    ...
                    >
                      <IconFileText
                        size={16}
                        stroke={1.5}
                        style={{ color: notePriorityColors[n.priority || 'medium'] }}
                      />
                      <span
                        style={{
                          overflow: 'hidden',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            {selectedNote ? (
              ...
              <div className="empty-state">
                ...
                <Text size="sm" color="gray.5">
                  Выберите заметку слева или создайте новую для совместной работы
                </Text>
              </div>
            )}
          </div>
