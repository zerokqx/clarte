...
description: 'Link workspace packages in monorepos (npm, yarn, pnpm, bun). USE WHEN: (1) you just created or generated new packages and need to wire up their dependencies, (2) user imports from a sibling package and needs to add it as a dependency, (3) you get resolution errors for workspace packages (@clarte/*) like "cannot find module", "failed to resolve import", "TS2307", or "cannot resolve". DO NOT patch around with tsconfig paths or manual package.json edits - use the package manager''s workspace commands to fix actual linking.'
---
...
pnpm add @clarte/ui --workspace

# Or with --filter from anywhere
pnpm add @clarte/ui --filter @clarte/app --workspace
```

Result in `package.json`:

```json
{ "dependencies": { "@clarte/ui": "workspace:*" } }
```
...
```bash
yarn workspace @clarte/app add @clarte/ui
```
...
```bash
npm install @clarte/ui --workspace @clarte/app
```

Result in `package.json`:

```json
{ "dependencies": { "@clarte/ui": "*" } }
```
...
```bash
cd packages/app && bun add @clarte/ui
```
...
```bash
npm install @clarte/data-access @clarte/ui --workspace @clarte/dashboard
```
...
- Symlinks appear in `<consumer>/node_modules/@clarte/<package>`
