...
export const useNotes = () => {
  ...
  const createNote = useCallback(
    (
      ...
      const newNote: Note = {
        id:
          customId || `room-${Math.random().toString(36).substr(2, 9)}-${Date.now().toString(36)}`,
  ...
  const updateNotePriority = useCallback((id: string, priority: 'high' | 'medium' | 'low') => {
    setNotes((prev) =>
      prev.map((n) => (n.id === id ? { ...n, priority, updatedAt: new Date().toISOString() } : n)),
    );
  }, []);

  const addSharedNote = useCallback(
    (id: string, title = 'Совместная заметка', priority: 'high' | 'medium' | 'low' = 'medium') => {
      setNotes((prev) => {
