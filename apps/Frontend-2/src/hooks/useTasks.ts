...
export const useTasks = () => {
  ...
  const fetchTasks = async () => {
    ...
    setError(null);
    try {
      const response = await apiClient.get<RawTask[]>('/todos');
        ...
        const initialMockTasks = [
          {
            ...
            description:
              'Это ознакомительная задача. Вы можете отмечать задачи как выполненные, перемещать их и редактировать.',
            isCompleted: false,
          ...
          },
          {
            ...
            description:
              'Посмотрите спецификацию IDEF0.md в корне проекта для ознакомления с архитектурой системы.',
            isCompleted: false,
            dueDate: new Date().toISOString(),
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
      ...
      const mappedTasks: Task[] = fetchedTasks
        .filter((t: RawTask) => !deletedIds.includes(t.id))
        .map((t: RawTask) => {
          const meta = localMeta[t.id] || {};
          let section = meta.section;
  ...
  const addTask = async (taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      ...
      console.error(err);
      const axiosErr = err as AxiosError<{ details?: string; message?: string }>;
      const msg =
        axiosErr.response?.data?.details || axiosErr.response?.data?.message || axiosErr.message;
  ...
  const deleteTask = async (id: string) => {
    ...
    if (cached) {
      ...
      const updated = cachedTasks.filter((t: RawTask) => t.id !== id);
